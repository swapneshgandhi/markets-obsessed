...
class YahooFinanceApi(rTicker: String, hTicker: String, tickerId: String) extends QuandlApi(rTicker, tickerId) {
  ...
  override def getLatestQuote: RealTimeQuote = {
    ...
    val easternTimeZone = "America/New_York"

    val sdf = new SimpleDateFormat("MM/dd/yyyy hh:mma", Locale.US)
    sdf.setTimeZone(TimeZone.getTimeZone(easternTimeZone))
    val date = sdf.parse((response(DATE_COLUMN) + " " + response(TIME_COLUMN)).replaceAll("\"", ""))
